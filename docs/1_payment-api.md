...
商户业务系统通过统一下单接口发起支付收款订单，三希智付支付网关会根据商户配置的支付通道路由支付通道完成支付下单。支付网关根据不同的支付方式返回对应的支付参数，业务系统使用支付参数发起收款。

> 接口说明

适用对象：`普通商户` `特约商户`

请求URL：https://pay.sanxipay.com/api/pay/unifiedOrder
...
商品标题 | subject | 是 | String(64) | 三希智付商品标题测试 | 商品标题  
商品描述 | body | 是 | String(256) | 三希智付商品描述测试 | 商品描述  
...
异步通知地址 | notifyUrl | 否 | String(128) | https://www.sanxipay.com/notify.htm | 支付结果异步回调URL,只有传了该值才会发起回调  
跳转通知地址 | returnUrl | 否 | String(128) | https://www.sanxipay.com/return.htm | 支付结果同步跳转通知URL
...
分账模式 | divisionMode | 否 | int | 0 | 分账模式：<br> 0-该笔订单不允许分账[默认], <br> 1-支付成功按配置自动完成分账支付完成自动分账【需包含自动分账接收方，否则自动更新为0（V3.0.0新增）】 , <br> 2-商户手动分账(解冻商户金额)  
...
```json
{
  ...
  "notifyUrl": "https://www.sanxipay.com",
  "signType": "MD5",
  "currency": "cny",
  "returnUrl": "",
  "mchNo": "M1623984572",
...
支付数据 | payData | 否 | String | http://www.sanxipay.com/pay.html | 发起支付用到的支付参数，如果微信公众号支付等 
...
2.  `channelExtra`参数中添加 subAppId 参数，该参数为商户的公众号或小程序的AppId，示例：
...
请求URL：https://pay.sanxipay.com/api/pay/query
...
请求URL：https://pay.sanxipay.com/api/pay/close
...
请求URL：https://pay.sanxipay.com/api/channelUserId/jump
...
跳转地址 | redirectUrl | 是 | String | https://www.sanxipay.com | 获取到用户ID后，会携带用户ID参数跳转到该地址  
请求时间 | reqTime | 是 | long | 1622016572190 | 请求接口时间,13位时间戳  
接口版本 | version | 是 | String(3) | 1.0 | 接口版本号，固定：1.0  
签名 | sign | 是 | String(32) | C380BEC2BFD727A4B6845133519F3AD6 | 签名值，详见签名算法  
签名类型 | signType | 是 | String(32) | MD5 | 签名类型，目前只支持MD5方式 
...
```json
{
  ...
  "appId": "60cc09bce4b0f1c0b83761c9",
  "ifCode": "wxpay",
  "redirectUrl": "https://www.sanxipay.com",
...
```html
https://www.sanxipay.com/toU?channelUserId=o6BcIwvSiRpfS8e_UyfQNrYuk2LI
```
