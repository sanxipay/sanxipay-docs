...
***第一步：*** 设所有发送或者接收到的数据为集合M，将每个非空参数构造成完整的 `key=value&` 片段，再将这些片段按**不区分大小写的字典序**排序（Java 对应 `String.CASE_INSENSITIVE_ORDER`），依次拼接成字符串stringA。
特别注意以下重要规则：   
◆ 排序对象是完整的 `key=value&` 片段，不是只对参数名排序；
◆ 排序比较时不区分大小写，参数名和参数值原文仍须保持不变；
◆ 如果参数的值为空不参与签名；   
◆ 参数名区分大小写；   
