...
商户业务系统通过统一退款接口发起退款请求，三希智付支付网关会根据商户发起的支付订单号，找到对应到支付通道发起退款。

> 接口说明

适用对象：`普通商户` `特约商户`

请求URL：https://pay.sanxipay.com/api/refund/refundOrder
...
异步通知地址 | notifyUrl | 否 | String(128) | https://www.sanxipay.com/notify.htm | 退款完成后回调该URL,只有传了该值才会发起回调  
...
请求URL：https://pay.sanxipay.com/api/refund/query
...
商户号 | mchNo | 是 | String(30) | M1621873433953 | 商户号 
应用ID | appId | 是 | String(24) | 60cc09bce4b0f1c0b83761c9 | 应用ID
退款订单号 | refundOrderId | 二选一 | String(30) | R20160427210604000490 | 与mchRefundNo二者传一即可
商户退款单号 | mchRefundNo | 二选一 | String(30) | 20160427210604000490 | 与refundOrderId二者传一即可
请求时间 | reqTime | 是 | long | 1622016572190 | 请求接口时间,13位时间戳  
接口版本 | version | 是 | String(3) | 1.0 | 接口版本号，固定：1.0  
签名 | sign | 是 | String(32) | C380BEC2BFD727A4B6845133519F3AD6 | 签名值，详见签名算法  
签名类型 | signType | 是 | String(32) | MD5 | 签名类型，目前只支持MD5方式 
...
::: warning 商户必须完成的处理
先验签，再按 `mchRefundNo` 查询本地退款单并核对 `refundAmount`：`state=2` 更新为退款成功，`state=3` 更新为退款失败，其他状态拒绝处理。退款通知必须以 `refundOrderId` 独立幂等，不能与支付通知共用 `payOrderId` 作为唯一键；成功落库后返回 `success`，重复通知也返回 `success`，但不得重复增加退款金额或执行资金副作用。
:::
