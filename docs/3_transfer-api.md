...
商户业务系统通过转账接口发起转账申请，三希智付支付网关将根据请求数据传入到对应的上游接口。

> 接口说明

适用对象：`普通商户` `特约商户`

请求URL：https://pay.sanxipay.com/api/transferOrder
...
接口代码 | ifCode | 是 | String(10) | wxpay | wxpay-微信官方接口 ; alipay-支付宝官方接口; aliaqfpay-支付宝安全发接口
...
异步通知地址 | notifyUrl | 否 | String(128) | https://www.sanxipay.com/notify.htm | 转账完成后回调该URL,只有传了该值才会发起回调
...
请求URL：https://pay.sanxipay.com/api/transfer/query
...
请求URL：https://pay.sanxipay.com/api/transfer/balance/query
...
商户号 | mchNo | 是 | String(30) | M1621873433953 | 商户号 
应用ID | appId | 是 | String(24) | 60cc09bce4b0f1c0b83761c9 | 应用ID
支付接口 | ifCode | 是 | String(10) | aliaqfpay | aliaqfpay-支付宝安全发接口
请求时间 | reqTime | 是 | long | 1622016572190 | 请求接口时间,13位时间戳  
接口版本 | version | 是 | String(3) | 1.0 | 接口版本号，固定：1.0  
签名 | sign | 是 | String(32) | C380BEC2BFD727A4B6845133519F3AD6 | 签名值，详见签名算法  
签名类型 | signType | 是 | String(32) | MD5 | 签名类型，目前只支持MD5方式 
